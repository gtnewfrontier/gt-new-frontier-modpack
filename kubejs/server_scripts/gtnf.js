...
ServerEvents.recipes((event) => {
    ...
    woodTypesServer.forEach((woodObj) => {
        barrel(woodObj);
        ...
        chest(woodObj);
        // todo: add metal chests with sheets
    }, this);

    event.shapeless("minecraft:chest", Item.of(`sophisticatedstorage:chest`));

    event.shaped("minecraft:hopper", ["pcp", "pgp", "wph"], {
        p: "gtceu:iron_plate",
        c: "sophisticatedstorage:chest",
    ...
    event.remove({ output: "jeg:scrap" });

    event.shapeless("jeg:scrap", [
        "1x minecraft:iron_ingot",
        "1x gtceu:steel_ingot",
        "1x gtceu:bronze_ingot",
    ]);

    event.custom({
        type: "jeg:scrap_workbench",
        materials: [{ tag: "gtceu:circuits/lv", count: 1 }],
        result: { item: "jeg:circuit_board" },
        time: 5,
    });

    event.custom({
        type: "jeg:gunnite_workbench",
        materials: [{ tag: "gtceu:circuits/lv", count: 1 }],
        result: { item: "jeg:circuit_board" },
        time: 5,
    });
