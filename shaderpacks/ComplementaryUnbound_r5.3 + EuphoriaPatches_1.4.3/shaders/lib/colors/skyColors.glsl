    ...
    #define INCLUDE_SKY_COLORS
        ...
        vec3 skyColorNoon = skyColorM * (1.28 - 0.47 * rainFactorM);
        vec3 skyColorSunset = skyColorM * (1.18 - 0.44 * rainFactorM) * invNightFactor * invNightFactor;
        ...
        vec3 sunsetUpSkyColor     = skyColorSunset * (vec3(0.8, 0.58, 0.58) + vec3(0.1, 0.2, 0.35) * rainFactorM2);
        vec3 sunsetMiddleSkyColor = skyColorSunset * (vec3(1.8, 1.3, 1.2) + vec3(0.15, 0.25, -0.05) * rainFactorM2);
