...
| 9 | **Release and distribution** — versioning, changelog, CurseForge upload, playtest gate. **Decided 2026-09-08: `docs/design/01-gt-pin-and-distribution.md`.** GT is repinned to the newest CurseForge release, `gtceu-1.20.1-7.5.3.jar` (`metadata:curseforge`), because GT 8 for 1.20.1 has never had a stable release and upstream has not published it either — their last tag `v1.14.5` is on 7.5.1. The export now carries zero jars in `overrides/`. The GT 8 tree is preserved on the `gt8` branch. | 18 | **decided** |
