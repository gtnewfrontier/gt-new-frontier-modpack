...
| 9 | **Release and distribution** — versioning, changelog, CurseForge upload, playtest gate. **Unparked 2026-09-08: v0.5.0 is releasable, and there is a hard blocker.** `packwiz curseforge export` bundles `gtceu-…-269.jar` into `overrides/mods/` because task 11 pinned GT to a maven snapshot in url mode; CurseForge only permits bundling mods on its Approved Non-CurseForge list, and GTCEu Modern is a CurseForge project. Repin to a CF release, publish off CurseForge, or seek permission. | 18 | **open** |
